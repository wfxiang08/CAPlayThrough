...
// 从abls中拷贝数据到buffers中
// 最多拷贝: nbytes
inline void StoreABL(Byte **buffers, int destOffset,
                     const AudioBufferList *abl, int srcOffset, int nbytes) {
	int nchannels = abl->mNumberBuffers;
	const AudioBuffer *src = abl->mBuffers;
	while (--nchannels >= 0) {
		if (srcOffset > (int)src->mDataByteSize) continue;
		memcpy(*buffers + destOffset, (Byte *)src->mData + srcOffset,
               std::min(nbytes, (int)src->mDataByteSize - srcOffset));
		++buffers;
		++src;
	}
}
...
// 从buffers中拷贝数据到abls中
// 最多拷贝: nbytes
inline void FetchABL(AudioBufferList *abl, int destOffset,
                     Byte **buffers, int srcOffset, int nbytes) {
	int nchannels = abl->mNumberBuffers;
	...
	while (--nchannels >= 0) {
        ...
        // 从buffers中拷贝数据到abls中
        // 最多拷贝: nbytes
		memcpy((Byte *)dest->mData + destOffset, *buffers + srcOffset,
               std::min(nbytes, (int)dest->mDataByteSize - destOffset));
		++buffers;
		++dest;
	}
}
...
CARingBufferError CARingBuffer::Store(const AudioBufferList *abl,
                                      UInt32 framesToWrite, SampleTime startWrite) {
